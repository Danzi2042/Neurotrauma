...
public class firstAidItem extends AppCompatActivity {
    ...
    private void setInitialData() {
        ...
        operations.add( new Operation("Бинт, эластичный бинт\n антибиотический клей", "Производится в Медицинский фабрикатор, Используется для лечения Открытые раны, ожоги, предотвращает инфекцию; повязка и пластилин стабилизируют вывихи и переломы для последующего лечения, Важность Существенные.",
                R.drawable.bandage));
        ...
        operations.add( new Operation("Игла", "Производится в Медицинский фабрикатор, Используется для лечения Пневмоторакс. Это только мера первой помощи, Важность Важно.",
                R.drawable.spine));
        operations.add( new Operation("Гематологический анализатор и \n пустая карта донора", "Производится в Изготовитель, Используется для Определение группы крови и анализ крови, Важность Неважен.",
        ...
        operations.add( new Operation("Корректирующие очки", "Немного помогает со зрением (снимает нечеткость зрения) Занимает слот для головы.",
                R.drawable.glasses));
        operations.add( new Operation("Усовершенствованное устройство \nдля извлечения глазного яблока", "Ударьте им ползуна, мудраптора или молотоголава, чтобы получить его глаза Попадите в любого хаска или заражённых монстров, чтобы получить глаза хаска. Не может быть использован на людях или мертвых существах Ломается после 1 использования Очень сложный в использовании.",
                R.drawable.spoon));
    }
